...
"${TON_BUILD_DIR}/lite-client/lite-client" \
    -p "${INITIATOR_KEYS_DIR}/liteserver.pub" \
    -a ${INITIATOR_HOSTNAME}:3031 \
    -rc "getconfig 1" -rc "quit" \
    &>"${ELECTIONS_WORK_DIR}/elector-addr"

awk -v TON_BUILD_DIR="${TON_BUILD_DIR}" -v INITIATOR_KEYS_DIR="${INITIATOR_KEYS_DIR}" -v ELECTIONS_WORK_DIR="${ELECTIONS_WORK_DIR}" '{
    if (substr($1, length($1)-13) == "ConfigParam(1)") {
        printf TON_BUILD_DIR "/lite-client/lite-client ";
        printf "-p " INITIATOR_KEYS_DIR "/liteserver.pub -a ${INITIATOR_HOSTNAME}:3031 ";
...
if [ "$election_id" == "0" ]; then
    ...
    "${TON_BUILD_DIR}/lite-client/lite-client" \
        -p "${INITIATOR_KEYS_DIR}/liteserver.pub" -a ${INITIATOR_HOSTNAME}:3031 \
...
"${TON_BUILD_DIR}/validator-engine-console/validator-engine-console" \
    -k "${INITIATOR_KEYS_DIR}/client" \
    -p "${INITIATOR_KEYS_DIR}/server.pub" \
    -a ${INITIATOR_HOSTNAME}:3030 \
    -c "newkey" -c "quit" \
...
awk -v validator="${VALIDATOR_NAME}" -v wallet_addr="$MSIG_ADDR" -v TON_BUILD_DIR="${TON_BUILD_DIR}" \
    -v INITIATOR_KEYS_DIR="${INITIATOR_KEYS_DIR}" -v ELECTIONS_WORK_DIR="${ELECTIONS_WORK_DIR}" -v TON_SRC_DIR="${TON_SRC_DIR}" '{
    if (NR == 1) {
        election_start = $1 + 0
        ...
        printf "-k " INITIATOR_KEYS_DIR "/client -p " INITIATOR_KEYS_DIR "/server.pub -a ${INITIATOR_HOSTNAME}:3030 ";
...
awk -v validator="${VALIDATOR_NAME}" -v TON_BUILD_DIR="${TON_BUILD_DIR}" -v INITIATOR_KEYS_DIR="${INITIATOR_KEYS_DIR}" -v ELECTIONS_WORK_DIR="${ELECTIONS_WORK_DIR}" '{
    if (NR == 2) {
        request = $1
    } else if (($1 == "created") && ($2 == "new") && ($3 == "key")) {
        printf TON_BUILD_DIR "/validator-engine-console/validator-engine-console ";
